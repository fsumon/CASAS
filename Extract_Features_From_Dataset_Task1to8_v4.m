%% Importing CASAS dataset to create initial loader file

%{
    Importing CASAS dataset to create initial loader file to extract features.
    The files are trimed for task duration 1-8 as there are over 20
    tasks performed by the participants. Motion sensor events were also
    calculated based on only task 1-8.
%}
...
for j = 1 : nfiles
    ...
    strCell{1} = str;
    %% Triming the file to only keep the data for task 1-8.
    cellIndex = ~cellfun('isempty',strfind(strCell{1}," 9-start"));
    %% Only keeping the file content when Task 9 is about to start
    strCut{1} = strCell{1}(1:find(cellIndex));
...
%%  Calculate duration of each IADL tasks
    ...
    for tasks = 1 : 8
        startTask = ~cellfun('isempty',strfind(strCut{1}," "+tasks+"-start"));
        endTask   = ~cellfun('isempty',strfind(strCut{1}," "+tasks+"-end"));
        ST = strCut{1}(startTask);
        ET = strCut{1}(endTask);
...
%%  Counting sensor firing events  
        for sensors = 1 : 51
            ...
            searchStr =  " " + sensor + " " + "ON";
            idx = ~cellfun('isempty',strfind(strCut{1},searchStr));
